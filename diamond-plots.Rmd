...
```{r echo=TRUE, fig.cap="Comparing female and male participants." }
...
```{r echo=FALSE, fig.cap="Figure 4 in the manuscript.", fig.width=9, fig.height=9 }
...
```{r echo=TRUE, fig.cap="A duo comparison diamond plot for comparison of female and male participant based on a rich perspective on the data." }
...
ggsave(file.path(outputPath, 'figure 5.png'),
       duoComparisonDiamondPlot(dat,
                                items=c('highDose_AttBeliefs_hallucinate',
                                        'highDose_AttBeliefs_timeAwareness',
                                        'highDose_AttBeliefs_memory',
...
a <- diamondPlot(data.frame(ci.lo = c(-.31, -.16, .23, .37, .58),
                            es = c(-.18, -.02, .35, .48, .66),
                            ci.hi = c(-.05, .12, .47, .58, .73)),
                 yLabels=c('Hallucinate', 'Time', 'Memory', 'Health', 'Experience'));
...
require("ggplot2");
ggsave(plot = a,
       file = path.expand("~/../Desktop/figure.png"),
       width=10, height=10, units='cm', dpi=300, type="cairo");
