...
```{r echo=TRUE, fig.cap="The same diamond plot, but showing the raw datapoints." }
meansDiamondPlot(dat, items=c('highDose_AttBeliefs_hallucinate',
                              'highDose_AttBeliefs_timeAwareness',
                              'highDose_AttBeliefs_memory',
                              'highDose_AttBeliefs_health',
...
```{r echo=FALSE, fig.cap="Figure 1 in the manuscript.", fig.width=9, fig.height=9 }

fig1_panelA <- meanSDtoDiamondPlot(labels = c('Hallucination',
                               'Time',
                               'Memory',
                               'Health',
                               'Experience'),
...
```{r echo=TRUE, fig.cap="Figure 2 in the manuscript." }
...
ggsave(file.path(outputPath, 'figure 2.png'),
       biAxisDiamondPlot(dat, selectedVarNames,
                  leftAnchors[selectedVariables],
                  rightAnchors[selectedVariables],
                  subQuestions=subQuestions[selectedVariables]),
...
```{r echo=FALSE, fig.cap="Figure 3 in the manuscript.", fig.width=9, fig.height=9 }
#associationMatrix(dat, selectedVarNames, 'gender');

fig3_panelA <- associationsDiamondPlot(dat,
                                       selectedVarNames,
                                       'highDose_attitude');
  
fig3_panelB <- associationsDiamondPlot(dat, selectedVarNames,
                                       criterion='highDose_attitude',
                                       alpha=.5, color = 'blue') +
...
fig3_panelC <- associationsDiamondPlot(dat,
                                       selectedVarNames,
                                       'highDose_attitude',
                                       generateColors=c("red", "green"),
                                       fullColorRange=c(-1, 1));

fig3_panelD <- associationsDiamondPlot(dat,
                                       selectedVarNames,
                                       'highDose_attitude',
                                       decreasing=FALSE);

fig3 <- rbind(cbind(arrangeGrob(top=textGrob("A", hjust=0, x=0,
                                             gp = gpar(fontsize=16)),
                                ggplotGrob(fig3_panelA)),
                    arrangeGrob(top=textGrob("B", hjust=0, x=0,
                                             gp = gpar(fontsize=16)),
                                ggplotGrob((fig3_panelB)))),
              cbind(arrangeGrob(top=textGrob("C", hjust=0, x=0,
                                             gp = gpar(fontsize=16)),
                                ggplotGrob(fig3_panelC)),
                    arrangeGrob(top=textGrob("D", hjust=0, x=0,
                                             gp = gpar(fontsize=16)),
                                ggplotGrob((fig3_panelD)))));

grid.newpage();
grid.draw(fig3);

ggsave(file.path(outputPath, 'figure 3.png'),
       fig3,
       width=20, height=18, units='cm',
       dpi=600);
